...
37. **Hosting.** Vercel was tried first. The Vercel connector could not create projects (403), and a CLI `--temporary` deploy would have put database and AI keys into an unowned deployment, so both apps run on **Render** in the team member's workspace, Singapore region (closest to Supabase `ap-south-1`). The backend is a Docker service (`backend/Dockerfile`, health check `/api/health`). The frontend is a Node service in same-origin mode. `vercel.json` + `backend/Dockerfile.vercel` stay in the repo for a later Vercel setup.
38. **Lock file.** `frontend/package-lock.json` was regenerated with npm 10 (what Node 20 on Render uses), because the npm 11 lock omitted Linux-only optional packages (`@emnapi/*`) and `npm ci` failed.

## Roadmap models (being built after deployment)

39. **TOPSIS ranking** replaces the weighted sum for non-produce packs (switchable with `engine.ranking-method`). Criteria and weights are the same four as before, so the priority modes keep their meaning. Barrier uses 1 + log10 of the safety margin capped at the over-engineering limit (20), so "more barrier than needed" doesn't win by itself. The weighted total is still computed and returned for transparency.
40. **Sleeping free services (502 after idle).** Render's free plan sleeps services after ~15 min; the backend then needs ~60–75 s to start, while the frontend's proxy answers 502 at once. Fix: the frontend API client retries 502/503/504 and network errors every 4 s for up to 150 s and shows a "Waking up the server" banner, and every page pings `/api/health` on load. There's also an opt-in keep-alive (`KEEPALIVE_ENABLED`, IST hour window) in the backend. It's off by default: two always-on services need ~1440 h/month, more than the 750 free hours per workspace. A GitHub Actions pinger wasn't used because the available token lacks the `workflow` scope, and it would have the same hours problem.
41. **Final hosting split.** Frontend on **Vercel** (free, never sleeps, so the PPT link opens instantly) and backend on **Render** (Docker). The Vercel project uses root directory `frontend/`, `NEXT_PUBLIC_API_URL=/` and `API_PROXY_TARGET=https://annkavach-backend.onrender.com`: Vercel forwards `/api/*` to Render, so the browser sees one origin. The earlier all-in-Vercel `services` config was removed.
